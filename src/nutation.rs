...
pub fn get_delta_psi(jd: &JulianDay) -> f64 {
    ...
    ))]
    {
        ...
        unsafe { delta_psi_avx2_fma(t, d, m, mp, f, omega) }
    }
...
/// Scalar-only variant of [`get_delta_psi`], always bypasses the AVX2 path.
///
/// Intended for benchmarking: lets callers measure the scalar loop directly
/// alongside `get_delta_psi` to quantify the AVX2 speedup.
#[must_use]
pub fn get_delta_psi_scalar(jd: &JulianDay) -> f64 {
    let t = (jd.get_value() - 2_451_545.0) / 36_525.0;
    let t2 = t * t;
    let t3 = t2 * t;
...
unsafe fn delta_psi_avx2_fma(t: f64, d: f64, m: f64, mp: f64, f_arg: f64, omega: f64) -> f64 {
    ...
    let vd = unsafe { _mm256_set1_pd(d) };
    let vm = unsafe { _mm256_set1_pd(m) };
    let vmp = unsafe { _mm256_set1_pd(mp) };
    let vf = unsafe { _mm256_set1_pd(f_arg) };
    let vo = unsafe { _mm256_set1_pd(omega) };
    let vt = unsafe { _mm256_set1_pd(t) };
    let mut vsum = unsafe { _mm256_setzero_pd() };
    ...
    for i in 0..chunks {
        ...
        unsafe { _mm256_storeu_pd(args.as_mut_ptr(), varg) };
        let vsins =
            unsafe { _mm256_set_pd(args[3].sin(), args[2].sin(), args[1].sin(), args[0].sin()) };
        ...
        let vsp = unsafe { _mm256_set_pd(r[3].6, r[2].6, r[1].6, r[0].6) };
        let vamp = unsafe { _mm256_fmadd_pd(vsp, vt, vs) };

        vsum = unsafe { _mm256_fmadd_pd(vamp, vsins, vsum) };
    }

    let mut lanes = [0.0f64; 4];
    unsafe { _mm256_storeu_pd(lanes.as_mut_ptr(), vsum) };
