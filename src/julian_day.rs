...
impl JulianDay {
    ...
    pub fn to_calendar_date(&self) -> Date {
        ...
        let f: f64 = jd_plus_half - f64::from(z);

        let a: i32 = if z < 2_299_161 {
            z
        } else {
            let alfa = ((f64::from(z) - 1_867_216.25_f64) / 36524.25) as i32;
            z + 1 + alfa - (f64::from(alfa) / 4_f64) as i32
        };

        let b: i32 = a + 1524;
        let c: i16 = ((f64::from(b) - 122.1_f64) / 365.25) as i16;
        let d: i32 = (365.25_f64 * f64::from(c)) as i32;
        let e: u8 = (f64::from(b - d) / 30.6001_f64) as u8;
...
pub fn get_julian_day(date: &Date) -> f64 {
    ...
    left_side as f64 + right_side as f64 + date.day + f64::from(b) - 1524.5_f64
}
...
mod test {
    ...
    fn test_sputnik_launch_date_to_julian_date() {
        ...
        assert_eq!(2_436_116.31, get_julian_day(&date));
    }
    ...
    fn test_1999_1_1() {
        ...
        assert_eq!(2_451_179.5, get_julian_day(&date));
    }

    #[test]
    fn test_1600_12_31() {
        let date = Date::new(1600, 12, 31.0);
        assert_eq!(2_305_812.5, get_julian_day(&date));
    }

    #[test]
    fn test_minus_1000_7_12_dot_5() {
        let date = Date::new(-1000, 7, 12.5);
        assert_eq!(1_356_001.0, get_julian_day(&date));
    }

    #[test]
    fn test_whole_jd_to_calendar_date() {
        // 2459581 is 2022-01-01 12:00:00 UTC
        let jd = JulianDay::new(2_459_581.0);
    ...
    fn test_fractional_jd_to_calendar_date() {
        ...
        let jd = JulianDay::new(2_436_116.31);
        let date = jd.to_calendar_date();
    ...
    fn test_more_jd_to_calendar_date() {
        ...
        let jd1 = JulianDay::new(1_842_713.0);
        ...
        let jd2 = JulianDay::new(1_507_900.13);
    ...
    #[test]
    fn test_jd_equality() {
        assert_eq!(JulianDay::new(2_436_116.31), JulianDay::new(2_436_116.31));
        assert!(JulianDay::new(2000.0) > JulianDay::new(1000.0));
        assert!(JulianDay::new(139_164.0) < JulianDay::new(2_451_911.0));
    }
}
